...
function [x, exit_flag, x_list] = bisection_solver_rec(fun, x_left, x_right, dxtol, ftol, max_iter)
    ...
    if fun(x) * fun(x_left) < 0
        [x, exit_flag] = bisection_solver_rec(fun,x_left,x,dxtol,ftol,max_iter);
    else
        [x, exit_flag] = bisection_solver_rec(fun,x,x_right,dxtol,ftol,max_iter);
    end
end